...
export const getVerifyCodeFunc: VerifyCodeFunType = ({ smsType = '', mobile = '' }) => axios.post(`/apis/v2/systemctl/sms/${ smsType }/verify/${ mobile }`)

// 校验手机短信是否可用
type checkMobileCodeData = { smsType: SmsType; mobile: string; verifyCode: string }
export const checkMobileCode = ({ smsType, mobile, verifyCode }: checkMobileCodeData) => axios.get(`/apis/v2/systemctl/sms/${ smsType }/verify/${ mobile }/${ verifyCode }/verification`)
