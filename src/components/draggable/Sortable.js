...
function sortableFactory () {
  ...
  var lastPointerElemX
  var lastPointerElemY
  ...
  var lastMode // 'swap' or 'insert'

  var targetMoveDistance

  // For positioning ghost absolutely
  var ghostRelativeParent
  var ghostRelativeParentInitialScroll = [] // (left, top)

  var realDragElRect // dragEl rect after current animation
  ...
  var IE11OrLess = !!navigator.userAgent.match(/(?:Trident.*rv[ :]?11\.|msie|iemobile)/i)
  var Edge = !!navigator.userAgent.match(/Edge/i)
  var FireFox = !!navigator.userAgent.match(/firefox/i)
  var Safari = !!(navigator.userAgent.match(/safari/i) && !navigator.userAgent.match(/chrome/i) && !navigator.userAgent.match(/android/i))
  ...
  var _detectDirection = function (el, options) {
    var elCSS = _css(el)
    var elWidth = parseInt(elCSS.width) -
					parseInt(elCSS.paddingLeft) -
					parseInt(elCSS.paddingRight) -
					parseInt(elCSS.borderLeftWidth) -
					parseInt(elCSS.borderRightWidth)
    var child1 = _getChild(el, 0, options)
    var child2 = _getChild(el, 1, options)
    var firstChildCSS = child1 && _css(child1)
    var secondChildCSS = child2 && _css(child2)
    var firstChildWidth = firstChildCSS && parseInt(firstChildCSS.marginLeft) + parseInt(firstChildCSS.marginRight) + _getRect(child1).width
    var secondChildWidth = secondChildCSS && parseInt(secondChildCSS.marginLeft) + parseInt(secondChildCSS.marginRight) + _getRect(child2).width
  ...
  /**
		 ...
		 */
  var _detectNearestEmptySortable = function (x, y) {
    for (var i = 0; i < sortables.length; i++) {
      if (_lastChild(sortables[i])) continue

      var rect = _getRect(sortables[i])
      var threshold = sortables[i][expando].options.emptyInsertThreshold
  ...
  var _isClientInRowColumn = function (x, y, el, axis, options) {
    var targetRect = _getRect(el)
    var targetS1Opp = axis === 'vertical' ? targetRect.left : targetRect.top
    var targetS2Opp = axis === 'vertical' ? targetRect.right : targetRect.bottom
    var mouseOnOppAxis = axis === 'vertical' ? x : y

    return targetS1Opp < mouseOnOppAxis && mouseOnOppAxis < targetS2Opp
  }

  var _isElInRowColumn = function (el1, el2, axis) {
    var el1Rect = el1 === dragEl && realDragElRect || _getRect(el1)
    var el2Rect = el2 === dragEl && realDragElRect || _getRect(el2)
    var el1S1Opp = axis === 'vertical' ? el1Rect.left : el1Rect.top
    var el1S2Opp = axis === 'vertical' ? el1Rect.right : el1Rect.bottom
    var el1OppLength = axis === 'vertical' ? el1Rect.width : el1Rect.height
    var el2S1Opp = axis === 'vertical' ? el2Rect.left : el2Rect.top
    var el2S2Opp = axis === 'vertical' ? el2Rect.right : el2Rect.bottom
    var el2OppLength = axis === 'vertical' ? el2Rect.width : el2Rect.height
  ...
  var _getWindowScrollingElement = function () {
    if (IE11OrLess) {
  ...
  var _scrollBy = function (el, x, y) {
    el.scrollLeft += x
    el.scrollTop += y
  }

  var _autoScroll = _throttle(function (/** Event */evt, /** Object */options, /** HTMLElement */rootEl, /** Boolean */isFallback) {
    ...
    if (options.scroll) {
      ...
      var winScroller = _getWindowScrollingElement()
  ...
  var _clearAutoScrolls = function () {
  ...
  var _checkAlignment = function (evt) {
  ...
  Sortable.prototype = /** @lends Sortable.prototype */ {
    ...
    _onTouchMove: function (/** TouchEvent */evt, forAutoScroll) {
      if (tapEvt) {
        ...
        var matrix = ghostEl && _matrix(ghostEl)
        var scaleX = ghostEl && matrix && matrix.a
        var scaleY = ghostEl && matrix && matrix.d
        var relativeScrollOffset = PositionGhostAbsolutely && ghostRelativeParent && _getRelativeScrollOffset(ghostRelativeParent)
        var dx = ((touch.clientX - tapEvt.clientX) +
							fallbackOffset.x) / (scaleX || 1) +
							(relativeScrollOffset ? (relativeScrollOffset[0] - ghostRelativeParentInitialScroll[0]) : 0) / (scaleX || 1)
        var dy = ((touch.clientY - tapEvt.clientY) +
    ...
    _appendGhost: function () {
      ...
      if (!ghostEl) {
        ...
        var rect = _getRect(dragEl, true, container, !PositionGhostAbsolutely)
        var css = _css(dragEl)
        var options = this.options
    ...
    _onDragOver: function (/** Event */evt) {
      ...
      if (activeSortable && !options.disabled &&
        ...
        if (!elLastChild || _ghostIsLast(evt, axis, el) && !elLastChild.animated) {
          ...
          var aligned = target.sortableMouseAligned
          var differentLevel = dragEl.parentNode !== el
          var side1 = axis === 'vertical' ? 'top' : 'left'
          var scrolledPastTop = _isScrolledPast(target, 'top') || _isScrolledPast(dragEl, 'top')
    ...
    _animate: function (prevRect, target) {
      ...
      if (ms) {
        ...
        if ((prevRect.left + prevRect.width / 2) !== (currentRect.left + currentRect.width / 2) ||
          ...
          var scaleX = matrix && matrix.a
          var scaleY = matrix && matrix.d
  ...
  function _find (ctx, tagName, iterator) {
    if (ctx) {
      var list = ctx.getElementsByTagName(tagName); var i = 0; var n = list.length
  ...
  function _ghostIsLast (evt, axis, el) {
    ...
    var mouseOnAxis = axis === 'vertical' ? evt.clientY : evt.clientX
    var mouseOnOppAxis = axis === 'vertical' ? evt.clientX : evt.clientY
    var targetS2 = axis === 'vertical' ? elRect.bottom : elRect.right
    var targetS1Opp = axis === 'vertical' ? elRect.left : elRect.top
    var targetS2Opp = axis === 'vertical' ? elRect.right : elRect.bottom
    var spacer = 10
  ...
  function _getSwapDirection (evt, target, axis, swapThreshold, invertedSwapThreshold, invertSwap, isLastTarget) {
    ...
    var mouseOnAxis = axis === 'vertical' ? evt.clientY : evt.clientX
    var targetLength = axis === 'vertical' ? targetRect.height : targetRect.width
    var targetS1 = axis === 'vertical' ? targetRect.top : targetRect.left
    var targetS2 = axis === 'vertical' ? targetRect.bottom : targetRect.right
    ...
    if (!invertSwap) {
      ...
      if (isLastTarget && targetMoveDistance < targetLength * swapThreshold) { // multiplied only by swapThreshold because mouse will already be inside target by (1 - threshold) * targetLength / 2
        ...
        if (!pastFirstInvertThresh) {
          ...
          var dragS2 = axis === 'vertical' ? dragRect.bottom : dragRect.right
