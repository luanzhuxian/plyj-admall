...
export default {
    ...
    // 初始化
        ...
        mousemove (e) {
            ...
            // 水平界限
            const horizontalLeft = this.borderHorizontalWidth * RATIO / scale
            // let horizontalRight = -(width * scale - this.cutWidth * RATIO - horizontalLeft * scale) / scale // 水平界限
            // 水平界限(上边注释的简写)
            const horizontalRight = -(width - this.cutWidth * RATIO / scale - horizontalLeft)
            // 左边界判断
            ...
            // 上下边界判断
            // 竖直界限
            const verticalTop = this.borderVerticalWidth * RATIO / scale
            // let verticalBottom = -(height * scale - this.cutHeight * RATIO - verticalTop * scale) / scale // 竖直界限
            // (上边代码的简写形式)
            const verticalBottom = -(height - this.cutHeight * RATIO / scale - verticalTop)
