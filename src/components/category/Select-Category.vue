<template>
    ...
    <div>
        ...
        <!-- 添加主分类 -->
        <EditMain v-if="categoryType === 0" @success="res => addSuccess(res, 1)" :show.sync="showAddCategory" />
        <EditSubset v-if="categoryType === 1 || categoryType === 2 || categoryType === 3" @success="res => addSuccess(res, 2)" :show.sync="showAddCategory" :parent="parent" />
    </div>
</template>
...
export default {
    ...
    computed: {
        ...mapGetters('goods', ['categoryTree', 'courseCategoryTree']),
        // 知识课程分类树
        knowledgeCourse () {
     ...
     */
        ...
        async init () {
            ...
            this.parent = JSON.parse(JSON.stringify(category))
            if (this.disabledIds[1]) {
