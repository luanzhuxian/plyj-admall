...
export default {
    ...
    methods: {
        getNameById (id) {
            const find = this.labelList.find(item => item.id === id)
        ...
        async add () {
            ...
            const tagsAlert = this.$route.name.includes('Courses') ? '课程标签不可超过6个字' : '商品标签不可超过6个字'
        ...
        async remove (label, index) {
            try {
                const data = await checkIsLabelUsed(label.id)
                if (data.result) {
        ...
        onCheckboxClick (id) {
            if (this.selected.length === 4 && !this.selected.includes(id)) {
                const tagsAlert = this.$route.name.includes('Courses') ? '课程标签不可超过4个' : '商品标签不可超过4个'
