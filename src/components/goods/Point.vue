  ...
  watch: {
    ...
    activities () {
      ...
      for (let item of this.activities) {
        let hashDom = document.getElementById(item.id)
        if (hashDom) {
  ...
  mounted () {
    ...
    setTimeout(() => {
      if (this.hash && this.hashDomList[this.hash]) {
        let top = this.hashDomList[this.hash].getBoundingClientRect().top
        this.scrollWrap.scrollTo(0, top)
      }
      this.addScroll()
    }, 500)
  ...
  methods: {
    ...
    otherScroll () {
      for (let id of this.ids) {
        ...
        let top = this.hashDomList[id].getBoundingClientRect().top
        if (top > 0 && top < 100) {
          this.hash = id
