...
export default {
  ...
  methods: {
    ...
    async getGoodsList (page = 1, type) {
      ...
      try {
        const { data: { result: { records, current, size, total } } } = await getGoods(this.goodsListForm)
    ...
    async setDefaultSelection () {
      for (const pro of this.goodsList) {
        for (const sku of pro.skuEntityList) {
          let ct = this.$refs.table.$refs[`child-table-${sku.id}`]
          if (ct) {
            ct.clearSelection()
          }
        }
    ...
    async setSelection (row) {
      for (const sku of this.defaultSelected) {
        const skuRow = row.skuEntityList.find(item => {
          return item.skuCode1 === sku.skuCode1 && item.skuCode2 === sku.skuCode2
        })
        await this.$nextTick()
    ...
    clearSelection () {
      ...
      for (const pro of this.goodsList) {
        ...
        table.toggleRowExpansion(pro, false)
      }
    },
    // 表格选择事件
