        ...
        >
            ...
            >
                <template slot="treeItemLabel" slot-scope="{ data }">
                    {{ `${data.categoryName}` }}{{ data.parentCode === '0' ? data.subcategoryCount > 0 ? `(${data.subcategoryCount})` : '' : '' }}
                </template>
                <template slot="default" slot-scope="{ data }">
                    <div class="category-ctrl">
                        <el-tooltip
                            ...
                            placement="top-start"
                        >
                            <pl-svg class="icon" name="icon-tianjiazifenlei" width="12" fill="#4F63FF" @click.stop="addChild(data)" />
                        </el-tooltip>
                        <el-tooltip
                            class="category-ctrl-item"
                            effect="dark"
                        ...
                        <el-tooltip
                            ...
                            placement="top-start"
                        >
                            <pl-svg class="icon" name="icon-shanchu1" width="12" fill="#4F63FF" @click.stop="remove(data)" />
                        </el-tooltip>
                    </div>
                </template>
            </pl-Tree>
            ...
            <pl-table :data="productList" @selection-change="selectionChange" @sortUpdate="sortUpdate">
                ...
                <el-table-column width="80px">
                    <template slot-scope="{ row }">
                        <img v-imgError v-viewer class="product-image" :src="row.productMainImage || row.graphicMainImg" alt="商品图片">
                    </template>
                </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getTree (type = '') {
      try {
        await this.$store.dispatch('GET_CLASSIFY_TREE', type)
      } catch (e) {
        throw e
      }
    },
