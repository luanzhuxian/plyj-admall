...
export default {
    ...
    data () {
        return {
            ...
            mobileOptions: [],
            selectedUsers: [],
            originalUsers: [], // 原有的所有学员信息
            cannotDeleteUserIds: [], // 已经接受送课的用户禁止删除的学员id列表
            isUpdate: false,
            userGroup: [],
                ...
                if (val) {
                    try {
                        if (this.liveId) {
                            ...
                            this.selectedUsers = JSON.parse(JSON.stringify(this.selectedUsersFromFather))
                            this.originalUsers = JSON.parse(JSON.stringify(this.selectedUsers))
                            // 已支付的或已观看的学员，不允许删除
                            this.cannotDeleteUserIds = this.originalUsers.filter(item => (item.isWatch || item.payAmount)).map(item => item.userId)
                        }
    ...
    // 获得用户分组标签和活动标签列表
        ...
        async getLiveSelectUsersInfo () {
            try {
                ...
                this.originalUsers = JSON.parse(JSON.stringify(this.selectedUsers))
                // 已支付的或已观看的学员，不允许删除
                this.cannotDeleteUserIds = result.users.filter(item => (item.isWatch || item.payAmount)).map(item => item.userId)
            } catch (e) {
                throw e
            }
        },
        ...
        async confirm () {
            try {
                const params = this.selectedUsers.map(item => {
                    const index = this.originalUsers.findIndex(iItem => iItem.userId === item.userId)
                    // 新加用户默认isWatch和isGive为0
                    item.isWatch = 0
                    item.isGive = 0
                    return index === -1 ? item : this.originalUsers[index]
                })
                if (this.liveId) { // 有liveId直接提交，没有的话，返回给父组件
                    await submitLiveSelectStudentList({
                        id: this.liveId,
                        params: {
                            users: params
                        }
