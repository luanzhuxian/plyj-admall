...
import { getUserByPhone, getSendLiveUsersInfo, submitSendLiveUsersList, getAllUserListByTagId } from './../../../../../../apis/product-center/online-teaching/live'
...
export default {
    ...
    // 获得用户分组标签和活动标签列表
        ...
        getWillNoAuthUserName () {
            const allGiveUsers = this.originalUsers.filter(item => item.isGive)
            const deleteGiveUsers = JSON.parse(JSON.stringify(allGiveUsers))
            for (const item of this.selectedUsers) {
                const index = deleteGiveUsers.findIndex(iItem => item.userId === iItem.userId)
                if (index !== -1) deleteGiveUsers.splice(index, 1)
            }
            return deleteGiveUsers.map(item => item.name || item.nickName).join('、')
        },
        async confirm () {
            const noAuthUserName = this.getWillNoAuthUserName()
            if (noAuthUserName) {
                await this.$confirm({
                    title: '温馨提示',
                    message: `用户${ noAuthUserName }取消赠送课程后，将无权限付费观看该直播。您可通过添加学员，开通用户付费观看直播的权限。`
                })
            }
            try {
                await submitSendLiveUsersList({
                    id: this.liveId,
                    params: {
