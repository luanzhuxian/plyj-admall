...
import { getLibraryList, deleteLibraryFile, uploadLibraryFile, getRemainingSpaceSize } from '../../../../../apis/line-teaching/video-library'
import { getVideoLibInfo } from '../../../../../apis/line-teaching/knowledge-course'
export default {
    ...
    methods: {
        ...
        async getVideoLibInfo () {
            try {
                const data = await getVideoLibInfo()
                this.teachingInfo = data.result
            } catch (e) {
                throw e
            }
        },
        ...
        async upload (urls, newList, fields) {
            ...
            for (const [i, { url, duration, size }] of newList.entries()) {
                ...
                const data = {
                    ...
                    fileSize: size,
                    // 视频时长
                    resourceTime: duration,
                    // 视频地址前缀
                    urlPrefix: url.slice(0, url.lastIndexOf('.')),
                    // 视频地址后缀
                    urlSuffix: url.slice(url.lastIndexOf('.'))
                }
