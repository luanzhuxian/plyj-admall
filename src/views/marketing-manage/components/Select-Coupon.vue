...
export default {
  ...
  watch: {
    async show (val) {
      if (val) {
        ...
        this.daterange = []
        let table = this.$refs.table // 隐藏弹框时，清空选中状态
  ...
  computed: {
    allSelect () {
      let allSelect = JSON.parse(JSON.stringify(this.selected))
      for (let item of this.originalSelected) {
        let index = this.findIndex(this.selected, item.id)
        let hadExist = allSelect.some(iItem => iItem.id === item.id)
        if (index === -1 && !hadExist) allSelect.push(item)
      }
      for (let item of this.newSelected) {
        let index = this.findIndex(this.selected, item.id)
        let hadExist = allSelect.some(iItem => iItem.id === item.id)
        if (index === -1 && !hadExist) allSelect.push(item)
      }
  ...
  methods: {
    async getList () {
      try {
        let { data } = await getCouponList(this.form)
    ...
    addNewCoupon () {
      let newCoupon = JSON.parse(sessionStorage.getItem('resultData'))
    ...
    resetSelectedCouponList () {
      ...
      let selectedList = JSON.parse(sessionStorage.getItem('selectedCouponList'))
      sessionStorage.removeItem('selectedCouponList')
      // 没有该记录，不影响正常操作
      if (!selectedList) return
      // 选出上次点击确认后，在本次影响后的结果
      this.originalSelected = this.initCouponArray.filter(item => {
        return this.findIndex(selectedList, item.id) !== -1
      })
      // 新建优惠券之前，新选择的
      this.newSelected = selectedList.filter(item => {
        return this.findIndex(this.originalSelected, item.id) === -1
      })
    },
    async defaultSelect () {
      if (this.originalSelected.length || this.newSelected.length) {
        ...
        for (let item of this.table) {
          // 是否应该被选中
          let willSelected = this.originalSelected.some(row => row.id === item.id)
          let willNewSelected = this.newSelected.some(row => row.id === item.id)
          // 是否已被选中
          let hasSelected = this.selected.some(row => row.id === item.id)
    ...
    deleteOriginalSelected (id) {
      let currentIndex = this.findIndex(this.originalSelected, id)
      if (currentIndex !== -1) this.originalSelected.splice(currentIndex, 1)
      let currentNewIndex = this.findIndex(this.newSelected, id)
