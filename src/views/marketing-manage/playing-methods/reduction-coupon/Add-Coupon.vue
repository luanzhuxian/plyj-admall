        ...
        <div class="add-content">
            <div class="content-title" id="basic" ref="basic">
                ...
                <div style="margin-left: 20px" class="purchase-sort-description">
                    满减券仅支持线上购买指定范围内的商品时，满足条件则自动抵扣支付金额；
                </div>
            </div>
            <el-form label-width="150px" :model="form" :rules="rules" ref="ruleForm" label-position="left" class="main-form">
                <el-form-item label-width="170px" label="满减券名称" prop="couponName">
                    <el-input style="width: 320px" @change="getBrief()" v-model="form.couponName" maxlength="50" placeholder="如，满减券（4-12个字）" />
                </el-form-item>
                <el-form-item label="满减券" required>
                    ...
                    <div class="man-l20" style="color: #999;font-size: 14px">
                        <p>满足金额需 >= 优惠金额，即可设置满100减10元满减券</p>
                        <p>满足大于0元可减金额，则购买商品满足此金额，即可减免相应的金额；</p>
                    </div>
                </el-form-item>
                <el-form-item>
                    <span slot="label">
                    ...
                    <div class="man-l20">
                        ...
                        <span class="purchase-sort-description"> （购买减免全场商品和课程金额）</span>
                    </div>
                </el-form-item>
                ...
                <el-form-item v-if="!form.distributionMethod && !status" label-width="170px">
                    ...
                    />
                    <span class="purchase-sort-description"> （满减券在指定的领取时间内，可正常显示和领取）</span>
                </el-form-item>
                <el-form-item v-if="!form.distributionMethod && status" label-width="170px">
                    <span slot="label">
                        <span>领取时间</span>
                ...
                <el-form-item label="使用须知" prop="brief">
                    <div class="flex" style="margin-left: 20px;">
                        <el-input
                            ...
                            :editable="false"
                        />
                        <pl-svg width="16" v-if="!form.briefEdit" fill="#4F63FF" name="icon-bianji1" class="brief-edit" @click="form.briefEdit = 1" />
                    </div>
                </el-form-item>
                <div class="content-title">
                    其他设置
                    ...
                    <div class="man-l20">
                        <el-radio-group v-model="form.scholarship">
                            <div class="use-limit" style="display: flex;flex-direction: column">
                                <el-radio :disabled="status" :label="1">
                                    可与奖学金合并使用<span class="purchase-sort-description"> （奖学金（即红包），是用户通过活动获取的；可支持购买时与满减券同时叠加使用）</span>
                                </el-radio>
                                <el-radio :disabled="status" :label="0">
                                    不可与奖学金合并使用<span class="purchase-sort-description"> （购买时使用该满减券时，不可选择奖学金叠加抵扣；）</span>
                                </el-radio>
                            </div>
                        </el-radio-group>
                        <div style="color: #999;font-size: 14px">
                            <br>
                            <p>秒杀、团购、预购活动均不支持使用满减券和奖学金进行减免；</p>
                        </div>
                    </div>
                </el-form-item>
            </el-form>
                            ...
                            <div class="coupon-item-left">
                                ...
                                <div class="life-time">
                                    ...
                                    <div class="desc-btn" @click="descShow = !descShow">
                                        ...
                                        <!--<pl-svg class="showInstruction" :name="descShow ? 'icon-shang' : 'icon-xia'" fill="#fff" width="10" />-->
                                    </div>
                                </div>
                            </div>
                            <div
...
export default {
    ...
    data () {
        return {
            ...
            rules: {
                ...
                couponName: [
                    { required: true, message: '满减券名称不能为空', trigger: 'blur' },
                    { max: 12, message: '满减券名称不能超过12个字符', trigger: 'blur' },
                    { min: 4, message: '满减券名称不能少于4个字符', trigger: 'blur' }
                ],
                useLimitAmount: [
    ...
    methods: {
        ...
        '1.优惠内容：全部商品可使用，满' + `${ this.form.useLimitAmount ? this.form.useLimitAmount : '*' }` + '元立减' + `${ this.form.amount ? this.form.amount : '*' }` + '元\n' +
