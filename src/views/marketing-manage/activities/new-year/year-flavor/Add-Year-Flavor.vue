    ...
    <div class="add-product">
        <div class="add-content">
            ...
            >
                ...
                <el-form-item label="年味内容" prop="materialSchemeModels">
                    <div>
                        <span>已选择 {{ form.materialSchemeModels.length }} 个年味</span>
                        <el-checkbox
                        ...
                        <span class="purchase-sort-description">（请选择活动界面需要展示的年味）</span>
                    </div>
                ...
                <el-form-item label="抽奖条件">
                    集齐已勾选的全部年味，即可获得1次抽奖机会
                </el-form-item>

                <el-form-item label="抽奖时间">
                    用户集齐指定年味，即可进行年味大礼的抽奖
                </el-form-item>
                ...
                <el-form-item label="总中奖率" prop="winningProbability">
                    ...
                    <span class="purchase-sort-description">（总中奖率仅针对年味大礼抽奖概率的设置）</span>
                    <el-tooltip popper-class="winning-probabilit" placement="top" effect="light">
                        <ul class="win-rate" slot="content">
                            <li>总中奖率仅针对年味大礼的抽奖，不针对阶梯奖品。</li>
                            <li>设置可分为2种情况：</li>
                            <li>
                                情况1：如果年味大礼每个参与用户均可有机会获得奖品；
                                设置中奖率为100%，则每个抽奖用户，均有机会获得
                                年味大礼，抽奖的概率按照库存数量进行计算，库存量
                                越少抽中概率越低，库存越多抽中概率越高；
                            </li>
                ...
                <el-form-item label="年味大礼" prop="giftModels">
                    <el-table
                        class="table mb-20"
                 ...
                 -->
                    ...
                    <span class="purchase-sort-description ml-10">（仅可支持添加至多3个年味大礼）</span>
                </el-form-item>
          ...
          预览
            ...
            <div class="preview">
                ...
                <img src="https://mallcdn.youpenglai.com/static/admall/2.2.0/c47870de-616c-4f6a-9b5d-8c9a02187dd1.png">
            </div>
        </div>
        <!--
      最小库存限制，
    ...
    data () {
        ...
        return {
            ...
            form: {
                ...
                // 使用说明
                activityRule:
          `1.在活动有效期内，获得20个年味，即可有机会参与年味大礼的抽奖
2.每人仅有1次抽取年味大礼的机会；
3.奖品：用户抽奖年味大奖的方式，采取的是随机抽奖的方式；
4.获得的奖品将自动存入“我的礼品”中，兑换有效期内用户可随时查看；
5.“我的礼品”中存放的年味大奖需在该礼品使用有效期内到店兑换；过期或已使用将不予兑换，请妥善保管；
6.用户参与活动过程中，获得指定数量的年味，还可额外获得优惠券、奖学金或小礼品；
...
8.优惠券和奖学金在有效期内未使用，将自动过期，过期后购买商品将不可进行抵扣。`,
            ...
            rules: {
                ...
                materialSchemeModels: [
                    { validator: checkMaterial, message: '年味选择不能少于7个', trigger: 'blur' }
                ],
                activityRule: [
    ...
    // 阶梯礼品添加对象，最多6个
        ...
        getBrief () {
            ...
            this.form.activityRule = `1.在活动有效期内，获得${ this.materialSchemeModels.length }个年味，即可有机会参与年味大礼的抽奖；
2.每人仅有1次抽取年味大礼的机会；
3.奖品：用户抽奖年味大奖的方式，采取的是随机抽奖的方式；
4.获得的奖品将自动存入“我的礼品”中，兑换有效期内用户可随时查看；
5.“我的礼品”中存放的年味大奖需在该礼品使用有效期内到店兑换；过期或已使用将不予兑换，请妥善保管；
...
8.优惠券和奖学金在有效期内未使用，将自动过期，过期后购买商品将不可进行抵扣。`
        ...
        async save () {
            try {
                ...
                // 参数变化
                this.form.logImgUrl = this.logImgUrl.join('')
                // 添加或者复制
                if ((this.id && this.isCopy) || !this.id) {
                    await addSigninActivity(this.form)
                    this.$success('添加成功')
                } else {
                    // 编辑
                    this.form.id = this.id
                    if (this.status === 0) {
                        await editActivityInfoNotStart(this.form)
                    }
                    if (this.status === 1) {
                        await editActivityInfoDataStart(this.form)
                    }
                    this.$success('编辑成功')
                }
                await this.$router.push({ name: 'YearFlavorList' })
            } catch (e) {
                if (e) throw e
            }
        },
