<template>
    <div class="together">
        <div class="together-top">
            <div class="together-top-left">
                ...
                <el-button
                    type="primary"
                    size="mini"
                    @click="$router.push({name:'AddBooking'})"
                    class="add-btn"
                >
        ...
        <pl-tabs
            value="BookingBuyList"
            :tabs="[
                {label:'活动列表',name:'BookingBuyList'},
                {label:'活动设置',name:'BookingBuySetting'},
            ]"
                ...
                <template slot-scope="{ row }">
                    <Operating>
                        <template slot="button-box">
                            <a
                                @click="$router.push({name:'BookingBuyDetail', params:{id: row.id}})"
                            >
                                详情
                            </a>
                            <a
                            ...
                            <a
                                @click="$router.push({name:'BookingBuyData', params:{id: row.id}})"
                            >
                                数据
                            </a>
                            <!-- 未开始/已结束的活动均支持删除 -->
...
export default {
    ...
    methods: {
        ...
        async getList () {
            try {
                const { data: res } = await bookingActivityPage(this.filterForm)
        ...
        async handleCopy (id) {
            try {
                ...
                this.$router.push({ name: 'AddBooking', params: { id: data.result.id, type: 'duplicate' } })
            } catch (e) {
                throw e
            } finally {
                this.getList()
