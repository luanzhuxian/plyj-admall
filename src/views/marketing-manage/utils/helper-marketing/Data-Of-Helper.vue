    ...
    <div>
        ...
        <div>
            <el-table :data="tableData" class="mt-10" stripe>
                ...
                >
                    <template #default="{ row }">
                        {{ row.payDiscounts }}（自购：{{ row.payDiscounts }} 分享：{{ row.payDiscounts }}）
                    </template>
                </el-table-column>
                <el-table-column
                    prop="payDiscounts"
                ...
                <el-table-column label="操作">
                    <template #default="{ row }">
                        <div class="operationList">
                            <span @click="watchOrderList(row)">查看商品订单</span>
                            <span @click="watchOrderList(row)">查看知识课程订单</span>
                            <span @click="watchOrderList(row)">查看课程订单</span>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
...
import { getUserListOfHelper, exportUserListOfHelper } from '../../../../apis/marketing-manage/helper'
...
export default {
    ...
    // 获取列表数据
        async getList () {
            try {
                const { data: { result: { records, total } } } = await getUserListOfHelper(this.filterForm)
        ...
        async exportData () {
            try {
                const dataList = await exportUserListOfHelper(this.filterForm)
